...
if (!MONGODB_URI) {
  throw new Error('MONGODB_URI environment variable is not set in .env.local. Please check your MongoDB connection string.');
}
...
async function connectToDatabase(): Promise<Mongoose> {
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(MONGODB_URI).then((mongooseInstance) => {
      console.log('Connected to MongoDB successfully'); // Success message after connection
